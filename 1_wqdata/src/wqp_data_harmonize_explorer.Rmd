...
doc.dissolved %>%
  ...
  summarize(count=n()) %>% 
  arrange(desc(count)) %>%
  kable(.,'html',caption='All doc sample methods and their count (678 methods)') %>%
  kable_styling() %>%
  scroll_box(width='600px',height='400px')
```
...
doc.dissolved %>%
  ...
  summarize(count=n()) %>% 
  arrange(desc(count)) %>% 
  kable(.,'html',caption='All DOC analytical methods') %>%
  kable_styling() %>%
  scroll_box(width='600px',height='400px')
...
doc.filtered %>% 
  ...
  summarize(count=n()) %>%
  arrange(desc(count)) %>%
  kable(.,'html',caption='All doc units') %>%
  kable_styling() %>%
  scroll_box(width='600px',height='400px')
...
doc.unit.drop <- doc.filtered %>%
  anti_join(doc.lookup,by='units') 

print(paste('We dropped',nrow(doc.unit.drop),'DOC observations because the units do not make sense'))
...
chl.a %>%
   ...
   summarize(count=n()) %>% 
   arrange(desc(count)) %>%
   kable(.,'html',caption='All chlorophyll sample methods and their count') %>%
   kable_styling() %>%
   scroll_box(width='600px',height='400px')
...
chl.a.fraction %>%
  ...
  summarize(count=n()) %>% 
  arrange(desc(count)) %>%
  kable(.,'html',caption='All chlorophyll analytical methods and their count') %>%
  kable_styling() %>%
  scroll_box(width='600px',height='400px')
...
Wow that is great. The vast majority of data is in a sensible unit (ug/L). We'll harmonize the remaining mass/volume units (ppm,mg/L) and drop the NAs and % units. We'll harmonize Chlorophyll a to the most common unit in the data (ug/L). 
...
chl.unit.drop <- chl.filtered %>%
  anti_join(chl.lookup,by='units') 


print(paste('We dropped',round(100*(nrow(chl.unit.drop)/nrow(chl.filtered)),2),'% of Chl data because the units do not make sense'))
...
harmonized.datasets <- list(chl.a.harmonized,doc.harmonized,tss.harmonized,sand.harmonized,secchi.harmonized)

names(harmonized.datasets) <- c('chl.a','doc','tss','sand','secchi')


## Initially we combined all of these datasets here, but this made computation down the line needlessly slow. 
# wqp.long <- map_df(harmonized.datasets,rbind) %>%
#   mutate(index=1:nrow(.)) %>%
#   dplyr::select(-parameter,-units,-org,-org_id,-sample_method,-analytical_method,-particle_size,-media,-sample_depth,-sample_depth_unit,-fraction,-status,-conversion)


for(i in 1:length(harmonized.datasets)){
  write_feather(harmonized.datasets[[i]],path=paste0('1_wqdata/out/harmonized/',                                                    names(harmonized.datasets)[i],'_harmony.feather'))
}
