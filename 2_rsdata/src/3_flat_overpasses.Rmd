...
wrs.inv <- read_feather(sc_retrieve('2_rsdata/out/site_inventory_path_row.feather.ind')) %>%
  arrange(SiteID)

#This is the inventory of wrs data with source attached
unq.inv <- read_feather(sc_retrieve('2_rsdata/out/unique_site_inventory.feather.ind')) %>%
  arrange(SiteID) %>%
  filter(!is.na(source)) %>%
  select(-lat,-long)
...
wqp.lagos <- read_feather(sc_retrieve('1_wqdata/out/wqp_lagos_unity.feather.ind'))
...
cloud.raw <- read_feather(sc_retrieve('2_rsdata/out/clouds.feather.ind'))
