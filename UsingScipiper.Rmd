...
table(inv.combo$Constituent)
inv.combo <-inv %>%
  filter(Constituent %in% c('tss','chlorophyll','doc','secchi')) %>%
  group_by(MonitoringLocationIdentifier) %>%
  summarize(n=length(unique(Constituent)),
            obs=sum(resultCount)) %>%
  filter(n > 3)
...
##Full Water Quality Pull (20 hours or so)
```{r, eval=F}
...
```{r}
library(scipiper)
scmake('tasks_1_wqp_munge.yml','remake.yml')

debug(scipiper::loop_tasks)
scmake('1_wqdata/log/tasks_1_wqp_munge.ind')

```

##What the munging does
```{r}

state.doc <- list.files('1_wqdata/out/wqp',full.names = T,pattern='doc') %>%
  .[!grepl('.ind',.)]


dat <- feather::read_feather(state.doc[23])
names(dat)
