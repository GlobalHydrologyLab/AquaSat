...
present <- length(list.files('3_wq_rs_join/data/in/sr'))

times <- file.info(list.files('3_wq_rs_join/data/in/sr',full.names=T))

date.avg <- as.Date(times$ctime) %>% mean(.)
date.sd <- as.Date(times$ctime) %>% sd(.)

redownload = ifelse(date.avg == Sys.Date() & 
                present==length(sr_match) &
                date.sd == 0,
              TRUE,
              FALSE)

#Create a vector with the destination path for each download
down.paths <- paste0('3_wq_rs_join/data/in/sr/',sr_match$name)
