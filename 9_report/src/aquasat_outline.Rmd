...
knitr::include_graphics("9_report/src/Watersat_Drop_flow.png")
...
q.cut.percents = c(0,.3,.6,.8,.9,.95,1)

sr.long.same <- sr.long.median %>%
  mutate(tss.bin=cut(tss,
                     breaks=c(min(tss,na.rm=T),20,100,max(tss,na.rm=T)),
                     labels=c('< 20 mg/L','20-100 mg/L','>100 mg/L'))) %>%
  mutate(tss.keep=tss) %>%
  gather(key=parameter,value=value,-SiteID,-date_unity,
         -date,-band,-refl,-type,-tss.bin,-lat,-long,-tss.keep) %>%
  group_by(parameter) %>%
  filter(value > 0) %>%
  filter(log10(value) > (mean(log10(value),na.rm=T)-
                           sd(log10(value),na.rm=T)*2),
         log10(value) < (mean(log10(value),na.rm=T)+
                           sd(log10(value),na.rm=T)*2)) %>%
  mutate(quintiles=cut(log(value),
                       quantile(log(value),q.cut.percents),
                       labels=c('0-30%','30-60%','60-80%','80-90%','90-95%','>95%'))) %>%
  mutate(quintile.cut.value = cut(log(value),q.cut.percents)) %>%
  ungroup() %>%
  filter(!is.na(quintiles)) %>%
  mutate(quintiles=factor(quintiles,levels=c('0-30%','30-60%','60-80%','80-90%','90-95%','>95%')))

quantile.colors <- RColorBrewer::brewer.pal(length(unique(sr.long.same$quintiles)),'GnBu')

png(filename='9_report/figures/quantilebins.png',width=8,height=6,units='in',res=250)
  sr.long.same %>%
  group_by(parameter) %>%
...
png(filename='9_report/figures/tssbins.png',width=8,height=10,units='in',res=250)
sr.breakdown %>%
  filter(parameter != 'tss') %>%
  group_by(band,parameter,tss.bin,quintiles) %>%
  summarize(refl=median(refl,na.rm=T),
            count=n()) %>%
  ungroup() %>%
  filter(count > 20) %>%
  ggplot(.,aes(x=band,y=refl,group=tss.bin,color=tss.bin)) +
  ylim(0,1300) +
  geom_line()+
  facet_grid(quintiles~parameter) + 
  theme(legend.position = 'top',legend.direction = 'horizontal') 
dev.off()
...
green.mod <- function(df) {
  mod <- lm(log10(value)~log10(green),data=df)
}

sed.mod <- function(df) {
  mod <- lm(log10(value) ~ log10(tss.keep),data=df)
}

green.sed.mod <- function(df) {
  mod <- lm(log10(value) ~ log10(green) * log10(tss.keep),data=df)
}


sr.bin.green <- sr.bin.wide %>%
  group_by(parameter) %>%
  nest() %>%
  mutate(greenmods = map(data,green.mod)) %>%
  mutate(sedmods = map(data,sed.mod)) %>%
  mutate(green.sed.mod=map(data,green.sed.mod)) %>%
  mutate(sed.pred = map2(sedmods,data,predict)) %>%
  mutate(tidy.mod= map(green.sed.mod,glance)) 

unnest(sr.bin.green,tidy.mod)

library(ggpmisc)
formula <- y ~ x
png(filename='9_report/figures/green_param.png',width=4,height=8,res=250,units='in')
ggplot(sr.bin.wide, aes(x=green,y=value)) +
  geom_hex() + 
  scale_x_log10() + 
  scale_y_log10() + 
  facet_wrap(~parameter,ncol=1,scales='free_y') + 
  stat_smooth(method='lm',se=F,formula=formula) +
...
```{r}
png(filename='9_report/figures/tss_param.png',width=5,height=6,res=250,units='in')
ggplot(sr.bin.wide %>%
...
```{r}
detss <- sr.bin.green %>%
  unnest(data,sed.pred)  %>%
  mutate(decor.val = value-10^sed.pred)
...
names(sr.breakdown)
png(filename='9_report/figures/tss_param.png',width=4,height=8,res=250,units='in')
ggplot(sr.breakdown %>%
         filter(tss.keep > .01,
                tss.keep < 5000,
                !band %in% c('swir1','swir2')),
       aes(x=refl,y=value,color=log10(tss.keep))) +
  geom_point(shape=1) + 
  scale_y_log10() + 
  scale_x_log10(limits=c(50,5000)) + 
  facet_grid(parameter ~ band)+ 
  scale_color_gradient2_tableau(palette = "Red-Blue", space = "rgb",
  na.value = "grey50", guide = "colourbar",name='Log TSS')
...
sr.long.sf <- left_join(sr.long.same,eco.sf,by=c('SiteID')) %>%
  filter(!is.na(NA_L1NAME)) %>%
  mutate(l1=factor(NA_L1NAME,levels=c('NORTH AMERICAN DESERTS','SOUTHERN SEMI-ARID HIGHLANDS','MEDITERRANEAN CALIFORNIA','GREAT PLAINS','TEMPERATE SIERRAS','EASTERN TEMPERATE FORESTS','MARINE WEST COAST FOREST','NORTHERN FORESTS','NORTHWESTERN FORESTED MOUNTAINS','TROPICAL WET FORESTS')))


png(filename='9_report/figures/ecoregion_conc.png',width=9,height=7,units='in',res=250)
sr.long.sf %>%
  ...
  scale_y_log10()+
  ylab('Ecoregion Level 1 Name')
dev.off()
```



## Simpler differentiation plots
